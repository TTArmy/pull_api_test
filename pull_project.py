...
def fetch_time_series_data(api_key, start_date, end_date, base_currency, target_currencies, resolution, amount, places,
                           format_):
...
def get_env_variable(var_name):
    ...
    if var_name == "API_KEY":
        masked_value = value[0] + '*' * (len(value) - 4) + value[-3] + value[-2] + value[-1]
        return masked_value
    return value
...
if __name__ == "__main__":
    try:
        masked_api_key = get_env_variable("API_KEY")
        start_date, end_date = get_user_input()[1:3]
        ...
        if None in (masked_api_key, start_date, end_date, base_currency, target_currencies, resolution, format_):
            raise ValueError("One or more environment variables are not set.")

        time_series_data = fetch_time_series_data(masked_api_key, start_date, end_date, base_currency,
                                                  target_currencies,
